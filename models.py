...
class CytologyClassifier:
    def __init__(self, num_classes=3, lr=0.0001, device=None, architecture='resnet18', class_counts=None):
        ...
        if architecture == 'resnet18':
            ...
            raise ValueError(f"Nieobsługiwana architektura: {architecture}")
    ...
    def train(self, train_loader, val_loader=None, num_epochs=20, save_best_path=None):
        log_dir = f"runs/exp_{self.architecture}_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
        ...
        plt.plot(range(1, num_epochs + 1), losses, marker='o', label='Train Loss')
