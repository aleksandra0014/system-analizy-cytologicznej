...
def predict_vgg_probs(model_or_wrapper, image_path: str, device) -> np.ndarray:
...
def predict_fused_func(pipe, label_encoder, classifier,  unet, device,image_path, probs_output=False):
    ...
    probs = predict_vgg_probs(classifier, image_path, device)
