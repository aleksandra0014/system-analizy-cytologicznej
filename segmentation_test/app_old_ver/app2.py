...
def predict_masks(model, image_tensor, device, threshold_nuclei=0.3, threshold_cell=0.5):
    ...
    return [mask_cell, mask_nucleus]  # zwraca listę dwóch masek
...
def main():
    ...
    if uploaded_file:
        ...
        for (x1, y1, x2, y2) in boxes:
            ...
            mask_nucleus = cv2.resize(masks[1], (crop.shape[1], crop.shape[0])) * 255
            mask_cell = cv2.resize(masks[0], (crop.shape[1], crop.shape[0])) * 255

            nuclei_masks.append(mask_nucleus.astype(np.uint8))
        ...
        # === SIATKA KOMÓREK ===
        st.subheader("Wycięte komórki z maskami")

        cols = 3  # komórka, maska jądra, maska komórki
        rows = (len(crops) + 1) // 2
        ...
        for i in range(len(crops)):
            ...
            with row[1]:
                st.image(cv2.resize(nuclei_masks[i], (128, 128)), caption="Maska jądra", channels="GRAY")
            with row[2]:
