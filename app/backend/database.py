...
# WALIDATORY — wszędzie stringi zamiast ObjectId
VALIDATORS = {
    COLL["lekarze"]: {
        "validator": {
            "$jsonSchema": {
                ...
                "properties": {
                    "lekarz_uid": {"bsonType": "string"},   # np. uuid; opcjonalnie
    ...
    COLL["pacjenci"]: {
        "validator": {
            "$jsonSchema": {
                ...
                "properties": {
                    "pacjent_uid": {"bsonType": "string"},   # Twój identyfikator (np. wpisany w UI)
    ...
    COLL["slajdy"]: {
        "validator": {
            "$jsonSchema": {
                ...
                "properties": {
                    "slajd_uid": {"bsonType": "string"},        # uuid
                    ...
                    "bbox_gridfs_name": {"bsonType": ["string","null"]},  # nazwa pliku w GridFS
    ...
    COLL["komorki"]: {
        "validator": {
            "$jsonSchema": {
                ...
                "properties": {
                    "komorka_uid": {"bsonType": "string"},       # np. slajd_uid:cell_id
...
async def ensure_collections():
    """Tworzy kolekcje z walidatorami i indeksami (synchronicznie, jednorazowo przy starcie)."""
    sync = MongoClient(MONGO_URI)[MONGO_DB]
    ...
    for cname, cfg in VALIDATORS.items():
        if cname not in sync.list_collection_names():
            ...
            try:
                ...
                # starsze MongoDB może nie wspierać collMod dla validatora — ignoruj
                pass
